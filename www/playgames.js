...
//Метод перезапускает подключение к сервису
PlayGames.prototype.reconnect = function (success, failed) {
	return cordova.exec(success, failed, "PlayGamesPlugin", "reconnect", [null]);
};

//Метод выполняет полный дисконнект от сервиса
PlayGames.prototype.disconnect = function (success, failed) {
	return cordova.exec(success, failed, "PlayGamesPlugin", "disconnect", [null]);
};
